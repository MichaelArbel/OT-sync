...
%commandPre = 'C:\Users\tolga\source\repos\Super4PCS\x64\Release\Super4PCS.exe -i';
...
fid = fopen('registration.sh', 'w');

for i=3:length(kbestFolder)
    folder = folders{i};
    ...
    for j = 1:N
        ...
        for k=1:N
            ...
            command = [commandPre ' ' shape1 ' ' shape2 ' ' outfn];
            %system(command);
            fprintf(fid, '%s\n\n', command);
