...
def sinkhorn_loop( softmin, α_logs, β_logs, C_xxs, C_yys, C_xys, C_yxs, ε_s, ρ, 
                   ...
                   extrapolate=None, debias=True, last_extrapolation=True , a_x_0=0, b_y_0=0,a_y_0=0,b_x_0=0):
    ...
    if debias:
        a_x = λ * softmin(ε, C_xx, (α_log + a_x_0/ε) )  # OT(α,α)
        b_y = λ * softmin(ε, C_yy, (β_log + b_y_0/ε) )  # OT(β,β)
    a_y = λ * softmin(ε, C_yx, (α_log+ b_x_0/ε) )  # OT(α,β) wrt. a
    b_x = λ * softmin(ε, C_xy, (β_log+ a_y_0/ε) )  # OT(α,β) wrt. b
    i  = len(ε_s)-1
    ...
    #while not done:
        ...
        #err = torch.norm(a_y_old - a_y) + torch.norm(b_x_old - b_x)
        #print(err)
        #done =  err < 10.*ε
    ...
    #err = torch.norm(a_y_old - a_y) + torch.norm(b_x_old - b_x)
    #print('sinkhorn err:' + str(err.item()))
    if debias:
        return a_x, b_y, a_y, b_x
    else:
        return None, None, a_y, b_x

def sinkhorn_loop2( softmin, α_logs, β_logs, C_xxs, C_yys, C_xys, C_yxs, ε_s, ρ, 
                   jumps=[], kernel_truncation=None, truncate=5, cost=None,
                   extrapolate=None, debias=True, last_extrapolation=True , a_x_0=0, b_y_0=0,a_y_0=0,b_x_0=0):
    ...
    # Start with a decent initialization for the dual vectors:
    if  a_y_0 is None:
        a_y_0 = torch.zeros_like(α_log)
    if b_x_0 is None:
        b_x_0 = torch.zeros_like(β_log)

    if debias:
        a_x = λ * softmin(ε, C_xx, (α_log + a_x_0/ε) )  # OT(α,α)
        b_y = λ * softmin(ε, C_yy, (β_log + b_y_0/ε) )  # OT(β,β)
    a_y = λ * softmin(ε, C_yx, α_log, b_x_0/ε- 0.5)   # OT(α,β) wrt. a
    b_x = λ * softmin(ε, C_xy, β_log, a_y_0/ε-0.5)   # OT(α,β) wrt. b
    i  = len(ε_s)-1
    #for i, ε in enumerate(ε_s):  # ε-scaling descent -----------------------
    done = False
    while not done:
        ...
        if debias:
            at_x = λ * softmin(ε, C_xx, α_log + a_x/ε )  # OT(α,α)
            bt_y = λ * softmin(ε, C_yy, β_log + b_y/ε )  # OT(β,β)
        at_y = λ * softmin(ε, C_yx, α_log, b_x/ε- 0.5 )  # OT(α,β) wrt. a
        bt_x = λ * softmin(ε, C_xy, β_log, a_y/ε-0.5 )  # OT(α,β) wrt. b
    ...
    if last_extrapolation:
        # Last extrapolation, to get the correct gradients:
        if debias:
            a_x = λ * softmin(ε, C_xx, α_log.detach(), (a_x/ε- 0.5).detach() )
            b_y = λ * softmin(ε, C_yy, β_log.detach(),  (b_y/ε-0.5).detach() )

        # The cross-updates should be done in parallel!
        a_y, b_x = λ * softmin(ε, C_yx, α_log.detach() , (b_x/ε-0.5).detach() ), \
                   λ * softmin(ε, C_xy, β_log.detach() , (a_y/ε-0.5).detach() )
    err = torch.norm(a_y_old - a_y) + torch.norm(b_x_old - b_x)
    print('sinkhorn err:' + str(err.item()))
    if debias:
        return a_x, b_y, a_y, b_x
