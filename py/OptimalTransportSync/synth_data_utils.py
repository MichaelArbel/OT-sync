...
def cdistAngle(theta1, theta2):
    ...
    print('sin1:\n', cos1.transpose(axes=0))
    print(sin1)
    
    x = np.concatenate((cos1, sin1), axis=1)
    y = np.concatenate((cos2, sin2), axis=1)
    print(x)
    D = cdist(x,y,'cosine')

    return D

def create_angular_synth_graph(N,numParticles=16,completeness=1,maxNumModes=4):
    ...
    synthData = {"type":"unit-circle", "N":N, "numParticles":numParticles, "numModes":numModes, "numEdges":e, "completeness":completeness,"G":G, "I":I,
                 "Xs":Xs,"Ys":Ys,"YsWeight":YsWeight, "DistrGndTruth":DistrGndTruth, "Ratios":Ratios, "RatiosY":RatiosY,"RatiosYWeight":RatiosYWeight}

    return synthData

def plot_angular(theta, weights):
...
def plot_angular_synth_data(synthData):
    e = synthData["numEdges"]
    I = synthData["I"]
    Xs = synthData["Xs"]
    Ys = synthData["Ys"]
